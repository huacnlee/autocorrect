...
switch (platform) {
  ...
  case 'win32':
    ...
    break
  case 'darwin':
    localFileExisted = existsSync(join(__dirname, 'autocorrect-node.darwin-universal.node'))
    try {
      if (localFileExisted) {
        nativeBinding = require('./autocorrect-node.darwin-universal.node')
      } else {
        nativeBinding = require('autocorrect-node-darwin-universal')
      }
      break
    } catch {}
    switch (arch) {
